...
/**
 * Where downloaded PDFs land by default: a `dsh-pubmed-pdfs/` folder inside the
 * session workspace (the plugin process's cwd — DSH launches with the session
 * working directory), so the files sit next to your project and survive
 * restarts. Deliberately NOT os.tmpdir(), which DSH points at a per-process
 * random `Temp/dsh-XXXXXX` directory that changes between runs and gets cleaned
 * up by the OS. Overridable per call with `outDir`, or globally with PDF_DIR.
 */
function pdfDownloadDir() {
  const configured = process.env.DSH_PUBMED_PDF_DIR
  if (configured && String(configured).trim()) return String(configured).trim()
  return join(process.cwd(), 'dsh-pubmed-pdfs')
}
