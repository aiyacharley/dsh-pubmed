...
export function nlpExtractKeywords(article) {
  ...
  if (text.trim()) {
    ...
    for (const phrase of doc.match('#Noun+').out('array')) {
      ...
      // F2: a phrase with ANY stopword is debris ("they share", "host and
      // microbe" style fragments leak in through #Noun+ matches across clause
      // boundaries). Real concept phrases ("bile acid metabolism") are
      // stopword-free, so this is a safe quality gate.
...
export function nlpExtractRelations(article) {
  ...
  // F2 (round 3, A1): STRICT gate — any stopword anywhere in the span drops it,
  // aligned with the keyword layer's rule above. Round-2's leading/trailing
  // trim let mid-stopword spans through ("tract can chronically"); stopword-free
  // fragments are handled by the core's A2 semantic gate (endpoint ∈ keywords).
  const clean = (span) => {
  ...
  while ((m = re.exec(text))) {
    ...
    const source = clean(m[1])
    const target = clean(m[3])
