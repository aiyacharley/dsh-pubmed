...
function registerPubmedTools(ctx, deps) {
  ...
  // P3-D: normalize the Europe PMC gate ONCE (it was evaluated inline at four
  // scattered read sites) — same pattern as s2Enabled further down.
  const europepmcEnabled = deps.europepmcEnabled !== false
  ...
  // Shared MEDLINE fetch (efetch db=pubmed) — used by pubmed_fetch_articles and
  // by pubmed_graph_add's pmids form.
  async function fetchMedlineArticles(pmids, signal) {
    const body = await eutilsGet('efetch.fcgi', { db: 'pubmed', id: pmids.join(','), rettype: 'medline', retmode: 'text' }, signal)
  ...
  // P2-C: shared ESummary fetch — search_articles, search_papers' PubMed branch
  // and find_related each used to re-implement this parse. One helper, three
  // call sites.
  async function esummaryRows(ids, signal) {
    if (!ids || !ids.length) return []
    const sbody = await eutilsGet('esummary.fcgi', { db: 'pubmed', id: ids.join(','), retmode: 'json', tool: TOOL }, signal)
    let sr
    try { sr = JSON.parse(sbody) } catch (e) { sr = null }
    if (!sr || !sr.result || !sr.result.uids) return []
    const rows = []
    for (const uid of sr.result.uids) {
      const rec = sr.result[String(uid)]
      if (rec) rows.push(summaryFromEsummary(String(uid), rec))
    }
    return rows
  }
  ...
  //
  // P1: session-level resolution cache. Agent chains (search -> fetch_pdf_oa ->
  // fetch_fulltext -> graph_add) re-resolve the SAME ids at every hop, and each
  // hop costs 1-2 queued NCBI calls. ID mappings are global facts (not user
  // data), so a plugin-lifetime Map is correct — negative results are cached
  // too, so a PMID with no PMCID is asked exactly once per process.
  const idCache = new Map()
  function cachedId(kind, key, run) {
    const k = kind + ':' + key
    if (idCache.has(k)) return Promise.resolve(idCache.get(k))
    return run().then(
      (v) => { idCache.set(k, v); return v },
      (e) => { throw e }, // network errors are NOT cached — retry next call
    )
  }
  async function pmidToPmcid(pmid, signal) {
    return cachedId('pmid2pmcid', String(pmid), async () => {
  ...
  async function pmcidToPmid(pmcid, signal) {
    ...
    return cachedId('pmcid2pmid', bare, async () => {
  ...
  async function doiToPmid(doi, signal) {
    return cachedId('doi2pmid', String(doi), async () => {
  ...
  async function pmidToDoi(pmid, signal) {
    return cachedId('pmid2doi', String(pmid), async () => {
  ...
  // P1-B (v0.4.2 round 4): declared per-name instead of relying on each call
  // site passing the 6th arg — the param existed but NO tool ever passed it,
  // so batch flows that legitimately run 60-90s (live-measured: 4-source
  // search with S2 backoff, 10-id OA batches) were being killed at 60s.
  const TOOL_TIMEOUTS = {
    pubmed_fetch_pdf_oa: 180000,   // batch: per-id id-resolution + 3 OA sources + download
    pubmed_search_papers: 180000,  // up to 4 sources; S2 429 backoff can exceed 60s alone
    pubmed_graph_add: 180000,      // batched enrichment budget alone is 150s
    pubmed_fetch_articles: 180000, // AUTO_GRAPH enrichment on large batches
    pubmed_fetch_fulltext: 120000, // multi-id JATS fetches
  }
  function register(name, description, parameters, execute, render, timeoutMs) {
    const def = defineTool({
      ...
      timeoutMs: timeoutMs || TOOL_TIMEOUTS[name] || 60000,
  ...
  register('pubmed_search_articles', 'Search PubMed with full query syntax, field filters, date ranges, pagination, and optional brief summaries (ESearch + ESummary). KEYWORD-level search — use for complex field-tag/date/pubType queries; PREFER pubmed_search_papers for broad cross-source sweeps (PubMed + Europe PMC + OpenAlex de-duplicated, with citation counts); PREFER pubmed_pubtator_search when the question names a specific bioconcept or asks for articles supporting a drug/gene-disease relation.',
    ...
    async (args, exec) => {
      ...
      try {
        ...
        const epmFallback = isNetError(netErr) && europepmcEnabled && !(exec.signal && exec.signal.aborted)
      ...
      let summaries = []
      // schema defaults are NOT auto-applied — the documented default is true,
      // so an absent/undefined flag must fetch summaries (an explicit false opts out).
      const includeSummaries = args.includeSummaries !== undefined ? !!args.includeSummaries : true
      if (includeSummaries && ids.length) {
        summaries = await esummaryRows(ids, exec.signal)
      }
      return {
        query: args.query,
        appliedTerm: term,
  ...
  register('pubmed_fetch_articles', 'Fetch full article metadata by PMIDs — abstract, authors with affiliations, journal, DOI, MeSH terms, grants, publication types (EFetch MEDLINE). With AUTO_GRAPH on (default) results are ALSO auto-merged into the current session knowledge graph — no separate pubmed_graph_add needed.',
    ...
    async (args, exec) => {
      ...
      const articles = await fetchMedlineArticles(pmids, exec.signal)
  ...
  register('pubmed_find_related', 'Find similar articles, citing articles, or references for a given PMID via ELink, enriched with ESummary. Citation-network expansion starting FROM one known article; for concept-based discovery (articles about a bioconcept/relation you do not yet have) use pubmed_pubtator_search instead.',
    ...
    async (args, exec) => {
      ...
      // ELink sometimes echoes the source PMID back among its own links (notably
      // in pubmed_pubmed similar sets) — the source is never its own neighbor.
      const ids = asArray(db && db.links).map(String).filter((x) => String(x) !== String(args.pmid))
      ...
      const summaries = await esummaryRows(capped, exec.signal)
  ...
  function mergeIntoPaper(base, other) {
    ...
    const merged = {
      ...
      // OA signals survive the merge: a paper is open access if ANY source says
      // so, and the first concrete OA URL wins.
      isOpenAccess: base.isOpenAccess || other.isOpenAccess || undefined,
      inPmc: base.inPmc || other.inPmc || undefined,
      oaUrl: base.oaUrl || other.oaUrl || undefined,
      oaStatus: base.oaStatus || other.oaStatus || undefined,
      openAccessPdf: base.openAccessPdf || other.openAccessPdf || undefined,
      foundIn,
    }
    if (merged.citedByCount === undefined) delete merged.citedByCount
    if (merged.isOpenAccess === undefined) delete merged.isOpenAccess
    return merged
  }
  ...
  function unifiedFromEpmRow(r) {
    ...
    return {
      ...
      // Europe PMC's own OA flags — free to carry, tells the caller which hits
      // are worth handing to pubmed_fetch_pdf_oa.
      isOpenAccess: r.isOpenAccess === 'Y' || undefined,
      inPmc: r.inPMC === 'Y' || undefined,
      epmcId: r.id,
      epmcSource: r.source,
    }
  }
  ...
  // P2-B: input is an ALREADY-NORMALIZED s2 row (from s2Search) — re-running
  // normalizeS2Paper here would look for externalIds on the normalized object
  // and silently drop pmid/doi.
  function unifiedFromS2Paper(n) {
  ...
  function unifiedFromOpenAlexWork(w) {
    ...
    return {
      ...
      // OpenAlex already ships the best OA link — carrying it costs no extra
      // request and lets a search result be handed straight to a download.
      oaUrl: (w.best_oa_location && (w.best_oa_location.pdf_url || w.best_oa_location.landing_page_url)) || (w.open_access && w.open_access.oa_url) || undefined,
      oaStatus: (w.open_access && w.open_access.oa_status) || undefined,
    }
  }

  // ===================================================== pubmed_search_papers (E3)
  register('pubmed_search_papers', 'ONE search across PubMed + Europe PMC + OpenAlex (default) (+ optional Semantic Scholar), de-duplicated into a single ranked list — papers found on multiple platforms rank first, then per-platform relevance, then citations, then year. Per-source status is reported (one platform failing never kills the call). Results carry open-access flags (isOpenAccess / oaUrl / oaStatus) so you can tell which hits are freely downloadable — pass their DOIs/PMIDs to pubmed_fetch_pdf_oa (batch form) to get OA PDF links and optionally download them. OpenAlex is on by default (fast, key-free, all-field); add "s2" to sources for Semantic Scholar, or pass sources:["all"] for everything (s2 is opt-in: slower — ~3s/call without an S2 key — and uses the shared S2 budget). Optionally filter by year and re-sort by citations/year. Use this for broad topic sweeps; use the platform tools (search_articles / europepmc_search / search_s2) when you need their specific filters, sorting, or paging.',
    {
    ...
    async (args, exec) => {
      ...
      const lists = []
      const per = []
      // P3-B: one handler per source — adding a source is now one map entry.
      // Every handler returns { rows (unified shape), total } and applies its
      // own year pushdown; the shared yearInRange post-filter stays as the net.
      const SEARCH_SOURCES = {
        pubmed: async (c) => {
          const params = { db: 'pubmed', term: c.query, retmode: 'json', retmax: c.perSource, tool: TOOL }
          if (c.yearRange) Object.assign(params, { mindate: c.yearRange.lo + '/01/01', maxdate: c.yearRange.hi + '/12/31', datetype: 'pdat' })
          const body = await eutilsGet('esearch.fcgi', params, c.signal)
          const er = (JSON.parse(body).esearchresult) || {}
          const rows = (await esummaryRows(asArray(er.idlist).map(String), c.signal))
            .map(unifiedFromPubmedSummary).filter((p) => yearInRange(p.pubYear, c.year))
          return { rows, total: parseInt(er.count, 10) || undefined }
        },
        s2: async (c) => {
          if (!s2Enabled) throw new Error('s2 disabled (S2_ENABLED:false)')
          const s = await s2Search(c.query, { limit: c.perSource, year: c.year }, c.signal)
          const rows = s.rows.map(unifiedFromS2Paper).filter(Boolean).filter((p) => yearInRange(p.pubYear, c.year))
          return { rows, total: s.total || undefined }
        },
        openalex: async (c) => {
          const oaParams = new URLSearchParams({ search: c.query, 'per-page': String(c.perSource), select: 'id,doi,title,publication_year,cited_by_count,authorships,primary_location,ids,open_access,best_oa_location' })
          if (c.yearRange) oaParams.set('filter', 'from_publication_date:' + c.yearRange.lo + '-01-01,to_publication_date:' + c.yearRange.hi + '-12-31')
          const data = await jsonGet('https://api.openalex.org/works?' + oaParams.toString(), c.signal)
          const rows = asArray(data.results).map(unifiedFromOpenAlexWork).filter(Boolean).filter((p) => yearInRange(p.pubYear, c.year))
          return { rows, total: (data.meta && data.meta.count) || undefined }
        },
        europepmc: async (c) => {
          const epmQuery = c.yearRange ? '(' + c.query + ') AND PUB_YEAR:[' + c.yearRange.lo + ' TO ' + c.yearRange.hi + ']' : c.query
          const data = await epmSearchRaw(epmQuery, c.perSource)
          const rows = asArray(data.resultList && data.resultList.result).map(unifiedFromEpmRow).filter((p) => yearInRange(p.pubYear, c.year))
          return { rows, total: data.hitCount || undefined }
        },
      }
      for (const src of wanted) {
        const handler = SEARCH_SOURCES[src]
        if (!handler) continue
        try {
          const r = await handler({ query, perSource, year, yearRange, signal: exec.signal })
          lists.push(r.rows)
          per.push({ source: src, count: r.rows.length, total: r.total })
        } catch (e) {
          per.push({ source: src, count: 0, error: e.message })
        }
      }
    ...
    (args, value) => {
      ...
      for (const p of value.papers || []) {
        ...
        const oa = p.isOpenAccess ? ' 🟢OA' : ''
        lines.push('')
        lines.push('- [' + (p.pmid ? 'PMID ' + p.pmid : p.doi ? 'DOI ' + p.doi : p.source) + ']' + (p.pubYear ? ' (' + p.pubYear + ')' : '') + ' ' + (p.title || '') + oa + found)
        ...
        if (p.oaUrl) lines.push('    OA: ' + p.oaUrl)
      }
      if ((value.papers || []).some((p) => p.foundIn?.length > 1)) lines.push('\nMulti-platform hits rank first.')
      const oaCount = (value.papers || []).filter((p) => p.isOpenAccess).length
      if (oaCount) lines.push('🟢OA = ' + oaCount + ' of ' + (value.papers || []).length + ' have an open-access copy — pass their DOIs/PMIDs to pubmed_fetch_pdf_oa (batch form) for PDF links or to download.')
      return [{ type: 'text', text: lines.join('\n').trim() }]
    })
  ...
  // P2-B: one S2 search used by BOTH the pubmed_search_s2 tool and the 's2'
  // branch of pubmed_search_papers — the URL building / parse / error shape was
  // duplicated at two call sites and had already started to drift (year param).
  async function s2Search(query, opts, signal) {
    const o = opts || {}
    const q = new URLSearchParams({ query: String(query || '').trim(), limit: String(Math.min(100, Math.max(1, o.limit || 10))), fields: S2_FIELDS })
    if (o.year) q.set('year', String(o.year))
    if (o.openAccessOnly) q.set('openAccessPdf', '')
    const r = await s2Get('/graph/v1/paper/search?' + q.toString(), signal)
    let data
    ...
    const rows = asArray(data.data).map(normalizeS2Paper).filter(Boolean)
    return {
      total: data.total || rows.length,
      nextOffset: (data.offset != null && data.next != null) ? data.offset + data.next : undefined,
      rows,
    }
  }
  if (s2Enabled) {
  ...
  register('pubmed_search_s2', 'Search Semantic Scholar (200M+ papers, ALL fields — complements the biomedical-only PubMed tools; free API). Returns title, authors, year, venue, citation counts, and normalized ids (DOI/PMID/ArXiv/CorpusId). Note: the unauthenticated shared rate limit can throttle calls; throttled requests retry automatically.',
    ...
    async (args, exec) => {
      const s = await s2Search(args.query, { limit: parseInt(args.maxResults, 10) || 10, year: args.year, openAccessOnly: args.openAccessOnly }, exec && exec.signal)
      return { query: args.query, total: s.total, papers: s.rows, nextOffset: s.nextOffset }
    },
    (args, value) => {
  ...
  register('pubmed_fetch_fulltext', 'Fetch full-text article bodies. Accepts pmids, pmcids, or dois (provide exactly one group). Resolves IDs to PMC and extracts JATS to readable sections — a two-tier chain (NCBI PMC EFetch, then Europe PMC fullTextXML) covers EPMC-only OA copies too. Long articles page through `offset`/`maxCharacters` (use the returned nextOffset for the next slice). Best-effort: articles with no retrievable full text are reported unavailable.',
    {
    ...
    async (args, exec) => {
      ...
      const articles = []
      // P3-A: two-tier full-text chain (mirrors the upstream server) —
      //   tier 1: NCBI PMC EFetch (structured JATS)
      //   tier 2: Europe PMC fullTextXML (same JATS DTD; covers EPMC-only OA
      //           copies and NCBI hiccups). Zero new deps — jatsToSections
      //           parses both.
      const pushJats = (id, idType, source, pmcid, xml) => {
        const s = jatsToSections(xml)
        if (!s.title && !s.abstract && s.sections.length === 0) return false
        let full = ''
        ...
        articles.push({ id, idType, source, pmcid: String(pmcid), title: s.title || undefined, body: text, truncated: nextOffset !== undefined, totalLength, offset: start, nextOffset, sections: s.sections })
        return true
      }
      const tryPmcTier = async (pmcid, idType, id, tried) => {
        try {
          const bare = String(pmcid).replace(/^PMC/i, '')
          const xml = await eutilsGet('efetch.fcgi', { db: 'pmc', id: bare, rettype: 'xml', retmode: 'xml' }, exec.signal)
          tried.push('pmc')
          return pushJats(id, idType, 'pmc', pmcid, xml)
        } catch (e) {
          tried.push('pmc:error')
          return false
        }
      }
      const tryEpmcTier = async (pmcid, idType, id, tried) => {
        try {
          tried.push('europepmc')
          // httpGet returns { status, body } — the JATS is in `.body`.
          const r = await withNetRetry(() => httpGet(EPMC + '/rest/' + pmcid + '/fullTextXML', exec.signal, PT_TIMEOUT_PROBE))
          return pushJats(id, idType, 'europepmc', pmcid, r.body)
        } catch (e) {
          return false
        }
      }
      // Both tiers in order; an article is unavailable only when both miss.
      const fetchChain = async (pmcid, idType, id) => {
        const tried = []
        if (await tryPmcTier(pmcid, idType, id, tried)) return
        if (await tryEpmcTier(pmcid, idType, id, tried)) return
        articles.push({ id, idType, source: null, unavailable: 'not-found', triedTiers: tried })
      }
      // EPMC's own DOI→PMCID mapping (sometimes succeeds where the NCBI chain
      // has no link, and it feeds the tier-2 EPMC fullTextXML).
      const epmcPmcidForDoi = async (doi) => {
        try {
          const data = await jsonGet(EPMC + '/search?' + qs({ query: 'DOI:"' + doi + '"', format: 'json', resultType: 'core', pageSize: 1 }), exec.signal, PT_TIMEOUT_PROBE)
          const hit = asArray(data.resultList && data.resultList.result)[0]
          return (hit && hit.pmcid) || null
        } catch (e) { return null }
      }
      const resolvePmid = async (pmid) => {
        try { return await pmidToPmcid(pmid, exec.signal) } catch (e) { return null }
      }
      for (const doi of asArray(args.dois).slice(0, 10)) {
        try {
          const pmid = await doiToPmid(doi, exec.signal)
          let pmcid = pmid ? await pmidToPmcid(pmid, exec.signal) : null
          if (!pmcid) pmcid = await epmcPmcidForDoi(doi)
          if (pmcid) await fetchChain(pmcid, 'doi', doi)
          else articles.push({ id: doi, idType: 'doi', source: null, unavailable: 'no-pmc', triedTiers: [] })
        } catch (e) {
      ...
      for (const pmid of asArray(args.pmids).slice(0, 10)) {
        ...
        if (pmcid) await fetchChain(pmcid, 'pmid', pmid)
        else articles.push({ id: pmid, idType: 'pmid', source: null, unavailable: 'no-pmc', triedTiers: [] })
      }
      for (const pmcid of asArray(args.pmcids).slice(0, 10)) {
        await fetchChain(pmcid, 'pmcid', pmcid)
      }
      return { articles }
    },
    (args, value) => {
  ...
  // ===================================================== OA PDF discovery (v0.4.2)
  // Finds open-access PDF/landing links for a DOI or PMID, aggregating THREE
  // free sources (verified live): Unpaywall (authoritative OA status + best
  // location), Europe PMC `fullTextUrlList` (publisher/render links), and
  // OpenAlex `open_access`/`best_oa_location`. No PDF parsing here — the tool
  // returns the link list, and optionally downloads the bytes to a local file
  // (`download: true`). Parsing the PDF is deliberately left to the caller.
  const UNPAYWALL_API = 'https://api.unpaywall.org/v2'
  // Unpaywall requires a REAL email (placeholder addresses get HTTP 422). The
  // built-in noreply address works; UNPAYWALL_EMAIL overrides it.
  const UNPAYWALL_EMAIL = deps.unpaywallEmail || NCBI_EMAIL

  function normalizeDoi(raw) {
    const s = String(raw == null ? '' : raw).trim().replace(/^doi:\s*/i, '').replace(/^https?:\/\/(dx\.)?doi\.org\//i, '')
    return /^10\.\d{4,9}\//.test(s) ? s : ''
  }
  function normalizePmcid(raw) {
    const s = String(raw == null ? '' : raw).trim().replace(/^PMC/i, '')
    return /^\d+$/.test(s) ? 'PMC' + s : ''
  }

  // P3-C: single shape factory for OA locations — every location carries the
  // same core keys (url/pdfUrl/landingUrl/hostType/version/license) regardless
  // of source, and source-specific extras (availability/style/site/oaDate/
  // isBest) pass through untouched.
  function oaLoc(source, fields) {
    const f = fields || {}
    return {
      source,
      url: f.url || null,
  ...
  // --- source: Unpaywall (DOI → OA status + every OA location) ---
  async function oaFromUnpaywall(doi, signal) {
    if (!doi) return { status: null, locations: [] }
    const url = UNPAYWALL_API + '/' + encodeURIComponent(doi) + '?email=' + encodeURIComponent(UNPAYWALL_EMAIL)
    let data
    try {
      data = await jsonGet(url, signal, PT_TIMEOUT_PROBE)
    } catch (e) {
      // 404 = DOI unknown, 422 = malformed/placeholder email — both are real
      // answers ("no OA found"), not service failures.
      const m = /HTTP (404|422)/.exec(String(e.message || ''))
      if (m) return { status: null, locations: [], note: 'unpaywall ' + m[1] }
      throw e
    }
    const locs = asArray(data.oa_locations).map((l) => oaLoc('unpaywall', {
      url: l.url_for_pdf || l.url || null,
      pdfUrl: l.url_for_pdf || null,
      landingUrl: l.url_for_landing_page || l.url || null,
      hostType: l.host_type || null,
      version: l.version || null,
      license: l.license || null,
      oaDate: l.oa_date || null,
      isBest: !!l.is_best,
    }))
    return { status: { isOa: !!data.is_oa, oaStatus: data.oa_status || null, journal: data.journal_name || null, publisher: data.publisher || null, title: data.title || null, year: data.year || null }, locations: locs }
  }

  // --- source: Europe PMC core record (isOpenAccess + fullTextUrlList) ---
  async function oaFromEpmc(doi, pmcid, signal) {
    const q = doi ? 'DOI:"' + doi + '"' : (pmcid ? 'PMCID:' + pmcid : '')
    if (!q) return { status: null, locations: [] }
    const url = EPMC + '/search?' + qs({ query: q, format: 'json', resultType: 'core', pageSize: 1 })
    let data
    try { data = await jsonGet(url, signal, PT_TIMEOUT_PROBE) } catch (e) { throw e }
    const hit = asArray(data.resultList && data.resultList.result)[0]
    if (!hit) return { status: null, locations: [] }
    const urls = asArray(hit.fullTextUrlList && hit.fullTextUrlList.fullTextUrl)
    const locs = urls.filter((u) => u && u.url).map((u) => oaLoc('europepmc', {
      url: u.url,
      pdfUrl: u.documentStyle === 'pdf' ? u.url : null,
      landingUrl: u.documentStyle === 'html' ? u.url : null,
      availability: u.availability || null,
      style: u.documentStyle || null,
      site: u.site || null,
      license: u.license || null,
    }))
    return {
      status: { isOa: hit.isOpenAccess === 'Y', inEpmc: hit.inEPMC === 'Y', inPmc: hit.inPMC === 'Y', pmid: hit.pmid || null, pmcid: hit.pmcid || null, title: hit.title || null, journal: hit.journalTitle || null, year: hit.pubYear || null },
      locations: locs,
    }
  }

  // --- source: OpenAlex (open_access.oa_url + best_oa_location.pdf_url) ---
  async function oaFromOpenAlex(doi, signal) {
    if (!doi) return { status: null, locations: [] }
    const url = 'https://api.openalex.org/works/doi:' + encodeURIComponent(doi) + '?select=id,doi,title,publication_year,open_access,best_oa_location,locations'
    let data
    try { data = await jsonGet(url, signal, PT_TIMEOUT_PROBE) } catch (e) {
      if (/HTTP 404/.test(String(e.message || ''))) return { status: null, locations: [], note: 'openalex 404' }
      throw e
    }
    const oa = data.open_access || {}
    const seen = new Set()
    const locs = []
    for (const l of [data.best_oa_location, ...asArray(data.locations)]) {
      if (!l || !(l.pdf_url || l.landing_page_url)) continue
      const key = (l.pdf_url || '') + '|' + (l.landing_page_url || '')
      if (seen.has(key)) continue
      seen.add(key)
      locs.push(oaLoc('openalex', {
        url: l.pdf_url || l.landing_page_url,
        pdfUrl: l.pdf_url || null,
        landingUrl: l.landing_page_url || null,
        hostType: l.source && l.source.type ? l.source.type : null,
        version: l.version || null,
        license: l.license || null,
        isBest: l === data.best_oa_location,
      }))
    }
    return { status: { isOa: !!oa.is_oa, oaStatus: oa.oa_status || null, oaUrl: oa.oa_url || null, title: data.title || null, year: data.publication_year || null, openAlexId: data.id || null }, locations: locs }
  }

  // Merge the three sources into one de-duplicated, best-first location list.
  // The dedup key is the full URL minus the fragment — query strings are kept
  // because they can be load-bearing (EPMC's `?pdf=render` is a different
  // resource from the same path without it).
  function mergeOaLocations(groups) {
    const out = []
    const byKey = new Map()
    for (const g of groups) {
      for (const loc of asArray(g)) {
        const key = String(loc.pdfUrl || loc.url || '').toLowerCase().replace(/#.*$/, '')
        if (!key) continue
        const hit = byKey.get(key)
        if (hit) {
          // Same URL seen in another source: keep one entry, record both sources.
          if (!hit.alsoIn.includes(loc.source)) hit.alsoIn.push(loc.source)
          if (!hit.license && loc.license) hit.license = loc.license
          if (!hit.version && loc.version) hit.version = loc.version
          if (loc.isBest) hit.isBest = true
        } else {
          const entry = Object.assign({}, loc, { alsoIn: [loc.source] })
          byKey.set(key, entry)
          out.push(entry)
        }
      }
    }
    // PDF links first, then best-flagged, then the rest.
    return out.sort((a, b) => (b.pdfUrl ? 1 : 0) - (a.pdfUrl ? 1 : 0) || (b.isBest ? 1 : 0) - (a.isBest ? 1 : 0))
  }

  // A location counts as an open-access copy when it is a direct PDF, or the
  // source labelled it free (EPMC availability) / attached an OA license.
  function isOaLocation(loc) {
    return !!loc.pdfUrl || loc.availability === 'Free' || !!loc.license
  }
  // One round of the three OA sources. Returns merged locations + per-source
  // statuses/errors. Shared by lookupOa and its PMID-conversion retry.
  async function queryOaSources(doi, pmcid, signal) {
    const settled = await Promise.allSettled([
      oaFromUnpaywall(doi, signal),
      oaFromEpmc(doi, pmcid, signal),
      oaFromOpenAlex(doi, signal),
    ])
    const sourceErrors = []
    const groups = []
    const statuses = []
    settled.forEach((r, i) => {
      const name = ['unpaywall', 'europepmc', 'openalex'][i]
      if (r.status === 'fulfilled') {
        groups.push(r.value.locations)
        if (r.value.status) statuses.push(Object.assign({ source: name }, r.value.status))
        else if (r.value.note) sourceErrors.push({ source: name, note: r.value.note })
      } else {
        sourceErrors.push({ source: name, error: String((r.reason && r.reason.message) || r.reason) })
      }
    })
    return { locations: mergeOaLocations(groups), statuses, sourceErrors }
  }

  // One identifier's OA lookup: resolve ids, query all three sources in
  // parallel, merge + rank. Shared by the single and batch code paths.
  async function lookupOa(seed, signal) {
    let doi = normalizeDoi(seed.doi)
    let pmid = seed.pmid != null && String(seed.pmid).trim() !== '' ? String(seed.pmid).trim() : ''
    let pmcid = normalizePmcid(seed.pmcid)
    // Resolve pmid/pmcid → doi (and fill the other id) so all three sources can run.
    if (!doi && (pmid || pmcid)) {
      try {
        if (!pmcid && pmid) pmcid = normalizePmcid(await pmidToPmcid(pmid, signal))
        if (pmcid && !pmid) { try { pmid = String(await pmcidToPmid(pmcid, signal) || '') } catch (e) { /* best-effort */ } }
        if (pmid) { try { doi = normalizeDoi(await pmidToDoi(pmid, signal)) } catch (e) { /* best-effort */ } }
      } catch (e) { /* resolution is best-effort; PMC/EPMC sources still work */ }
    }

    let q = await queryOaSources(doi, pmcid, signal)

    // F3: a bare-digit "pmcid" is ambiguous — PMIDs and PMCIDs are both numeric,
    // and agents routinely pass a PMID into the wrong parameter. If the PMCID
    // form resolves to nothing, AUTO-CONVERT: resolve the number as a PMID and
    // re-query. (User-approved behavior; live-verified.)
    let converted = null
    if (pmcid && !doi && /^\d+$/.test(String(seed.pmcid || '').trim()) && !q.locations.length && !q.statuses.length) {
      const asPmid = String(parseInt(seed.pmcid, 10))
      try {
        const c2 = normalizePmcid(await pmidToPmcid(asPmid, signal))
        let d2 = ''
        if (c2 || asPmid) { try { d2 = normalizeDoi(await pmidToDoi(asPmid, signal)) } catch (e) { /* best-effort */ } }
        if (c2 || d2) {
          const q2 = await queryOaSources(d2, c2, signal)
          if (q2.locations.length || q2.statuses.length) {
            q = q2
            pmid = asPmid
            if (c2) pmcid = c2
            if (d2) doi = d2
            converted = { from: seed.pmcid, treated: 'pmid:' + asPmid, pmcid: c2 || null, doi: d2 || null }
          }
        }
      } catch (e) { /* conversion is best-effort; the F3 note below still fires */ }
    }

    // OA is asserted only on positive evidence — an explicit is_oa flag or an
    // actual OA location. A lone subscription link must not read as open access.
    const anyOa = q.statuses.some((s) => s.isOa === true) || q.locations.some(isOaLocation)
    // Conversion also failed: the number is neither a valid PMCID nor a PMID
    // with an OA copy — say so with the actionable hint (F3, final fallback).
    if (pmcid && !doi && !q.locations.length && !q.statuses.length) {
      q.sourceErrors.push({ source: 'europepmc', note: 'no record found for ' + pmcid + ' — this does not look like a valid PMC ID; if the number is actually a PMID, pass it via pmids' })
    }
    return {
      input: { doi: doi || null, pmid: pmid || null, pmcid: pmcid || null },
      isOpenAccess: anyOa,
      oaStatus: (q.statuses.find((s) => s.oaStatus) || {}).oaStatus || null,
      locationCount: q.locations.length,
      locations: q.locations,
      bestPdfUrl: (q.locations.find((l) => l.pdfUrl) || {}).pdfUrl || null,
      sources: q.statuses,
      sourceErrors: q.sourceErrors.length ? q.sourceErrors : undefined,
      converted: converted || undefined,
    }
  }

  // Try the ranked PDF locations in order: publishers often answer a PDF URL
  // with an HTML consent/paywall page, so a failure moves on to the next
  // candidate (arXiv, repository copies, EPMC render) instead of giving up.
  // `nameHint` (PMID/DOI) keeps saved files identifiable instead of "pdf.pdf".
  async function downloadBestPdf(locations, outDir, signal, nameHint) {
    const candidates = locations.filter((l) => l.pdfUrl).map((l) => l.pdfUrl)
    if (!candidates.length) return { ok: false, error: 'no direct PDF URL found (see locations for landing pages)' }
    if (typeof deps.downloadToFile !== 'function') return { ok: false, error: 'download unavailable in this mode (requires the npm bundle plugin)' }
    const attempts = []
    for (const url of candidates) {
      try {
        const dl = await deps.downloadToFile(url, { dir: outDir, signal, name: nameHint })
        const done = Object.assign({ ok: true, url }, dl)
        if (attempts.length) done.attempts = attempts
        return done
      } catch (e) {
        attempts.push({ url, error: String((e && e.message) || e) })
      }
    }
    // Every candidate was blocked (publisher consent wall / bot protection, or a
    // PMC render URL that only works in a browser). Say so in actionable terms:
    // the caller usually has a better route (PMC structured text, or the link in
    // a browser) than retrying here.
    return {
      ok: false,
      error: 'all ' + candidates.length + ' PDF link(s) failed — they are likely behind a consent wall or bot check (PMC/EPMC render URLs often need a browser). Open one of the links in `locations` manually, or use pubmed_fetch_fulltext for PMC structured text.',
      attempts,
    }
  }

  register('pubmed_fetch_pdf_oa', 'Find open-access (OA) PDF links for a DOI / PMID / PMCID — aggregates Unpaywall (authoritative OA status + every OA location), Europe PMC fullTextUrlList, and OpenAlex open_access. Accepts ONE id (doi/pmid/pmcid) or a BATCH (pmids[]/dois[]/pmcids[], up to 10) — use the batch form to turn a search result list into a download list in one call. Returns a de-duplicated link list per id (direct PDF first, then landing pages) with host type, version (submitted/accepted/published), license, and OA status. Optionally downloads each best PDF with download:true (the PDF is NOT parsed — extraction is left to the caller). For structured JATS text of PMC articles use pubmed_fetch_fulltext.',
    {
      doi: { type: 'string', description: 'A single DOI (e.g. "10.1038/nature12373") — provide exactly one of doi / pmid / pmcid / pmids / dois / pmcids' },
      pmid: { type: 'string', description: 'A single PubMed ID (resolved to a DOI/PMCID first)' },
      pmcid: { type: 'string', description: 'A single PMC ID (e.g. "PMC3531190")' },
      pmids: { type: 'array', items: { type: 'string' }, description: 'BATCH: PubMed IDs (max 10) — the batch form for turning a search hit list into an OA download list' },
      dois: { type: 'array', items: { type: 'string' }, description: 'BATCH: DOIs (max 10)' },
      pmcids: { type: 'array', items: { type: 'string' }, description: 'BATCH: PMC IDs (max 10)' },
      download: { type: 'boolean', default: false, description: 'Download the best PDF for each id to a local file (requires the bundle plugin; returns savedPath + byte count per id). Off by default — review the link list first' },
      outDir: { type: 'string', description: 'Directory for the downloaded PDFs. Default: ~/.dsh/dsh-pubmed-pdfs/ (the plugin cannot know your session workspace — an agent that knows it should pass outDir explicitly, e.g. <workspace>/dsh-pubmed-pdfs/)' },
    },
    async (args, exec) => {
      // ── Collect seeds: single-id form or batch form (mutually exclusive) ──
      const single = [args.doi, args.pmid, args.pmcid].filter((v) => v != null && String(v).trim() !== '')
      const batches = [['pmids', args.pmids], ['dois', args.dois], ['pmcids', args.pmcids]]
        .map(([k, v]) => [k, asArray(v).map((x) => String(x).trim()).filter(Boolean)])
        .filter(([, v]) => v.length)
      if (single.length && batches.length) throw new Error('Use either the single-id form (doi/pmid/pmcid) or the batch form (pmids/dois/pmcids) — not both')
      if (single.length > 1) throw new Error('doi / pmid / pmcid are mutually exclusive — provide exactly one')
      if (batches.length > 1) throw new Error('pmids / dois / pmcids are mutually exclusive — provide exactly one batch')
      if (!single.length && !batches.length) throw new Error('Provide one of doi / pmid / pmcid (single) or pmids / dois / pmcids (batch)')

      const MAX_BATCH = 10
      let seeds
      if (single.length) {
        if (args.doi && !normalizeDoi(args.doi)) throw new Error('Not a valid DOI: ' + args.doi)
        if (args.pmcid && !normalizePmcid(args.pmcid)) throw new Error('Not a valid PMC ID: ' + args.pmcid)
        seeds = [{ id: String(single[0]).trim(), idType: args.doi ? 'doi' : args.pmid ? 'pmid' : 'pmcid', seed: { doi: args.doi, pmid: args.pmid, pmcid: args.pmcid } }]
      } else {
        const [kind, ids] = batches[0]
        const capped = ids.slice(0, MAX_BATCH)
        seeds = capped.map((id) => ({ id, idType: kind === 'pmids' ? 'pmid' : kind === 'dois' ? 'doi' : 'pmcid', seed: kind === 'pmids' ? { pmid: id } : kind === 'dois' ? { doi: id } : { pmcid: id } }))
      }

      // Lookups run in parallel — the three sources are independent per id, and
      // the shared rate-limit queues keep the upstreams safe.
      const looked = await Promise.all(seeds.map((s) => lookupOa(s.seed, exec.signal)))
      const results = looked.map((r, i) => Object.assign({ id: seeds[i].id, idType: seeds[i].idType }, r))

      // Optional download step — bytes to disk only, no parsing. Sequential so
      // one slow/blocked publisher link doesn't fan out into a burst.
      if (args.download) {
        for (const r of results) {
          const hint = r.input.pmid ? 'PMID' + r.input.pmid : r.input.doi ? r.input.doi.replace(/[^\w.\-]+/g, '_') : r.input.pmcid || r.id
          r.download = await downloadBestPdf(r.locations, args.outDir, exec.signal, hint)
        }
      }

      const oaCount = results.filter((r) => r.isOpenAccess).length
      const withPdf = results.filter((r) => r.bestPdfUrl).length

      // Single-id form keeps the original flat shape (backward compatible);
      // the batch form adds `results[]` + a summary.
      if (seeds.length === 1 && single.length) {
        return Object.assign({}, results[0], { input: results[0].input })
      }
      return {
        batch: true,
        idType: seeds[0].idType,
        requested: seeds.length,
        truncated: batches.length && batches[0][1].length > MAX_BATCH ? batches[0][1].length : undefined,
        openAccessCount: oaCount,
        downloadableCount: withPdf,
        results,
      }
    },
    (args, value) => {
      // ── Batch rendering ──
      if (value.batch) {
        const L = ['**OA PDF lookup — ' + value.requested + ' id(s): ' + value.openAccessCount + ' open access, ' + value.downloadableCount + ' with a direct PDF link**']
        if (value.truncated) L.push('(truncated to ' + value.requested + ' of ' + value.truncated + ')')
        for (const r of value.results) {
          const label = (r.input.pmid ? 'PMID ' + r.input.pmid : r.input.doi ? 'DOI ' + r.input.doi : r.input.pmcid || r.id)
          L.push('')
          L.push((r.isOpenAccess ? '✅' : '❌') + ' ' + label + (r.oaStatus ? ' (' + r.oaStatus + ')' : '') + ' — ' + (r.bestPdfUrl ? r.bestPdfUrl : r.locationCount ? 'no direct PDF (landing pages only)' : 'no OA copy'))
          // F3 echo: the id was auto-converted — the agent must see the swap.
          if (r.converted) L.push('    ℹ️ input "' + r.converted.from + '" is not a valid PMC ID — resolved as ' + r.converted.treated + (r.converted.pmcid ? ' → ' + r.converted.pmcid : '') + (r.converted.doi ? ' / DOI ' + r.converted.doi : ''))
          if (r.download) L.push('    ' + (r.download.ok ? 'saved → ' + r.download.savedPath + ' (' + r.download.bytes + ' bytes)' : 'download failed: ' + r.download.error))
        }
        const notOa = value.results.filter((r) => !r.isOpenAccess).length
        if (notOa) L.push('', notOa + ' id(s) have no open-access copy — use pubmed_fetch_fulltext (PMC) or the abstract for those.')
        if (!args.download && value.downloadableCount) L.push('', 'Call again with download:true to save these PDFs (not parsed — extraction is up to you).')
        return [{ type: 'text', text: L.join('\n').trim() }]
      }
      // ── Single-id rendering (unchanged) ──
      const L = []
      const ids = [value.input.doi ? 'DOI ' + value.input.doi : null, value.input.pmid ? 'PMID ' + value.input.pmid : null, value.input.pmcid ? value.input.pmcid : null].filter(Boolean).join(' · ')
      L.push((value.isOpenAccess ? '✅ Open access' : '❌ No OA copy found') + (value.oaStatus ? ' (' + value.oaStatus + ')' : '') + ' — ' + ids)
      // F3 echo: the id was auto-converted — the agent must see the swap.
      if (value.converted) L.push('ℹ️ input "' + value.converted.from + '" is not a valid PMC ID — resolved as ' + value.converted.treated + (value.converted.pmcid ? ' → ' + value.converted.pmcid : '') + (value.converted.doi ? ' / DOI ' + value.converted.doi : ''))
      L.push('')
      L.push('**' + value.locationCount + ' location(s)** — direct PDF first:')
      for (const l of value.locations || []) {
        L.push('- ' + (l.pdfUrl ? '📄 PDF' : '🔗 landing') + ' [' + l.source + (l.alsoIn && l.alsoIn.length > 1 ? '+' + (l.alsoIn.length - 1) : '') + '] ' + (l.pdfUrl || l.url))
        const meta = [l.hostType, l.version, l.license, l.availability].filter(Boolean)
        if (meta.length) L.push('    ' + meta.join(' · '))
      }
      if (value.bestPdfUrl) L.push('', 'Best PDF: ' + value.bestPdfUrl)
      if (value.download) {
        L.push('', value.download.ok
          ? 'Downloaded → ' + value.download.savedPath + ' (' + value.download.bytes + ' bytes)'
          : 'Download not performed: ' + value.download.error)
        if (value.download.ok && Array.isArray(value.download.attempts) && value.download.attempts.length) {
          L.push('   (skipped ' + value.download.attempts.length + ' blocked link(s) first)')
        }
        if (!value.download.ok && Array.isArray(value.download.attempts) && value.download.attempts.length) {
          for (const a of value.download.attempts.slice(0, 4)) L.push('   ✗ ' + a.url.slice(0, 90) + ' — ' + a.error.slice(0, 120))
        }
      } else if (value.bestPdfUrl) {
        L.push('', 'To save the PDF locally, call again with download:true (the PDF is not parsed — extraction is up to you).')
      }
      if (value.sourceErrors && value.sourceErrors.length) L.push('', 'Source notes: ' + value.sourceErrors.map((e) => e.source + ' (' + (e.error || e.note) + ')').join('; '))
      return [{ type: 'text', text: L.join('\n').trim() }]
    })
  ...
  // F2 (round 3, A4): SINGLE source of truth for relation-span cleaning. An
  // earlier round defined a second strict `cleanRelationSpan` higher up in this
  // scope, which this declaration silently shadowed (function hoisting: the
  // later declaration wins) — the strict gate was dead code and fragments like
  // "tract can chronically" survived. Now there is exactly one, and it is the
  // STRICT gate: any stopword anywhere in the span drops it ("tract can
  // chronically", "microbiome by microbial"), because relation endpoints feed
  // the keyword graph and force-keep logic makes debris permanent.
  // NOTE: stopword-free fragments ("high levels", "potent", "response") are
  // handled by the A2 semantic gate below (endpoint ∈ article keywords), not
  // here — no stopword gate can catch those.
  function cleanRelationSpan(span, stopwords) {
    const words = String(span || '').toLowerCase().split(/\s+/).filter(Boolean)
    if (!words.length || words.length > 4) return ''
    if (words.some((w) => stopwords.has(w))) return ''
    return words.join(' ')
  }
  function basicRelations(a) {
    ...
    while ((m = re.exec(text))) {
      const source = cleanRelationSpan(m[1], STOPWORDS)
      const relation = m[2].toLowerCase()
      const target = cleanRelationSpan(m[3], STOPWORDS)
  ...
  // C (F2 round 3): HEURISTIC_RELATIONS:false short-circuits the heuristic
  // relation layer entirely (NLP + fallback) — users who want a pure curated
  // (PubTator-only) relation graph get it with zero debris risk.
  const extractRelations = deps.heuristicRelations === false
    ? () => []
    : (deps.extractRelations || basicRelations)
  const relationEndpointRequireKeyword = deps.relationEndpointRequireKeyword !== false
  ...
  async function mergeGraph(graph, articles) {
    ...
    for (const a of list) {
      ...
      // A2 (F2 round 3): an endpoint must ALSO be an article keyword (present in
      // kwIds) — syntax fragments like "tract can chronically" or "high levels"
      // are almost never keyword phrases, so this semantic gate kills the whole
      // debris class. Switch: RELATION_ENDPOINT_REQUIRE_KEYWORD (default on).
      const kwSet = relationEndpointRequireKeyword ? new Set(kwIds) : null
      for (const rel of extractRelations(a)) {
        const s = 'kw:' + rel.source
        const t = 'kw:' + rel.target
        if (kwSet && (!kwSet.has(s) || !kwSet.has(t))) continue
  ...
  function buildMermaid(gj, maxKeywords) {
    ...
    // Bring in the endpoints of directed relation edges so red arrows appear
    // even when the relation phrases are not among the top keywords.
    // A3 (F2 round 3): only endpoints with count > 0 are brought in — count-0
    // keyword endpoints are creation debris, and bringing every endpoint in
    // made maxKeywords moot for them. Concept endpoints are exempt (they are
    // always meaningful), so curated red edges keep rendering.
    ...
    for (const e of gj.edges) {
      ...
      if (n1 && !(n1.type === 'keyword' && !n1.count) && !chosen.has(e.source)) addNode(n1, n1.label)
      if (n2 && !(n2.type === 'keyword' && !n2.count) && !chosen.has(e.target)) addNode(n2, n2.label)
    }
  ...
  register('pubmed_graph_add', 'Incrementally add a batch of retrieved articles to the CURRENT SESSION knowledge graph (in-memory, per-session). Does NOT touch the user graph until pubmed_graph_commit is called. Keyword extraction and PubTator concept/relation enrichment run automatically — do NOT call pubmed_extract_keywords first. Use dryRun:true to preview what WOULD be added without mutating the graph.',
    {
      articles: { type: 'array', description: 'Article objects as returned by pubmed_fetch_articles. Provide articles OR pmids' },
      pmids: { type: 'array', items: { type: 'string' }, description: 'BATCH shortcut: PubMed IDs (≤200) — the articles are fetched automatically, then merged. One call instead of fetch_articles + graph_add' },
    ...
    async (args, exec) => {
      const key = sessionKeyOf(exec)
      const g = getSessionGraph(key)
      // P2-A: accept pmids directly — the agent no longer has to copy article
      // objects out of a fetch_articles result into the graph_add arguments.
      let articles = asArray(args.articles)
      if (!articles.length && Array.isArray(args.pmids) && args.pmids.length) {
        articles = await fetchMedlineArticles(asArray(args.pmids).map(String).filter(Boolean).slice(0, 200), exec.signal)
      }
      if (!articles.length) throw new Error('Provide article objects or pmids to add')
      if (args.dryRun) {
        const preview = JSON.parse(JSON.stringify(g))
        const r = await mergeGraph(preview, articles)
      ...
      const r = await withGraphLock(key, () => mergeGraph(g, articles))
