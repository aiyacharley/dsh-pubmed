...
function registerPubmedTools(ctx, deps) {
      ...
      for (const src of wanted) {
        if (src === 'pubmed') {
          try {
            ...
            const ids = asArray(er.idlist).map(String)
            const rows = await esummaryRows(ids, exec.signal)
          ...
          try {
            const s = await s2Search(query, { limit: perSource, year: year }, exec && exec.signal)
            const unified = s.rows.map(unifiedFromS2Paper).filter(Boolean).filter((p) => yearInRange(p.pubYear, year))
            lists.push(unified)
            per.push({ source: 's2', count: unified.length, total: s.total || undefined })
          } catch (e) {
            per.push({ source: 's2', count: 0, error: e.message })
          }
        } else if (src === 'openalex') {
          try {
            const oaParams = new URLSearchParams({ search: query, 'per-page': String(perSource), select: 'id,doi,title,publication_year,cited_by_count,authorships,primary_location,ids,open_access,best_oa_location' })
  ...
  async function oaFromUnpaywall(doi, signal) {
    ...
    const locs = asArray(data.oa_locations).map((l) => ({
      source: 'unpaywall',
  ...
  async function oaFromEpmc(doi, pmcid, signal) {
    ...
    const locs = urls.filter((u) => u && u.url).map((u) => ({
      source: 'europepmc',
      url: u.url,
