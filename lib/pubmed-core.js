...
function registerPubmedTools(ctx, deps) {
  ...
  async function lookupOa(seed, signal) {
    ...
    const locations = mergeOaLocations(groups)
    ...
    const anyOa = statuses.some((s) => s.isOa === true) || locations.some(isOaLocation)
    // A bare-numeric "PMCID" that resolves to nothing is usually a PMID passed
    // in the wrong parameter (F3) — say so instead of a misleading "no OA copy".
    if (seed.pmcid && !doi && !locations.length && !statuses.length) {
      sourceErrors.push({ source: 'europepmc', note: 'no record found for ' + pmcid + ' — this does not look like a valid PMC ID; if the number is actually a PMID, pass it via pmids' })
    }
    return {
      input: { doi: doi || null, pmid: pmid || null, pmcid: pmcid || null },
      isOpenAccess: anyOa,
      oaStatus: (statuses.find((s) => s.oaStatus) || {}).oaStatus || null,
      locationCount: locations.length,
      locations,
      bestPdfUrl: (locations.find((l) => l.pdfUrl) || {}).pdfUrl || null,
      sources: statuses,
