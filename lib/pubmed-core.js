...
function registerPubmedTools(ctx, deps) {
  ...
  register('pubmed_search_papers', 'ONE search across PubMed + Europe PMC (+ optional Semantic Scholar), de-duplicated into a single ranked list — papers found on multiple platforms rank first, then per-platform relevance, then citations, then year. Per-source status is reported (one platform failing never kills the call). Add "s2" to sources to include Semantic Scholar (opt-in: slower — ~3s/call without an S2 key — and uses the shared S2 budget). Use this for broad topic sweeps; use the platform tools (search_articles / europepmc_search / search_s2) when you need their specific filters, sorting, or paging.',
    {
      ...
      sources: { type: 'array', items: { type: 'string', enum: ['pubmed', 'europepmc', 's2'] }, description: 'Which platforms to query; default = pubmed+europepmc (s2 is opt-in)' },
    },
    async (args, exec) => {
      const query = String(args.query || '').trim()
      if (!query) throw new Error('No query provided')
      const perSource = Math.min(50, Math.max(1, parseInt(args.maxResultsPerSource, 10) || 10))
      const wanted = (Array.isArray(args.sources) && args.sources.length) ? args.sources.filter((s) => ['pubmed', 'europepmc', 's2'].includes(s)) : ['pubmed', 'europepmc']
      if (!wanted.length) throw new Error('sources has no valid values — allowed: pubmed/europepmc/s2')
      const lists = []
      const per = []
      for (const src of wanted) {
        if (src === 'pubmed') {
          try {
            const body = await eutilsGet('esearch.fcgi', { db: 'pubmed', term: query, retmode: 'json', retmax: perSource, tool: TOOL }, exec.signal)
            ...
            lists.push(rows.map(unifiedFromPubmedSummary))
            per.push({ source: 'pubmed', count: rows.length, total: parseInt(er.count, 10) || undefined })
          } catch (e) {
          ...
          try {
            ...
            lists.push(rows.map(unifiedFromS2Paper).filter(Boolean))
            per.push({ source: 's2', count: rows.length, total: data.total || undefined })
          } catch (e) {
            per.push({ source: 's2', count: 0, error: e.message })
          }
        } else {
          try {
            const data = await epmSearchRaw(query, perSource)
            const rows = asArray(data.resultList && data.resultList.result)
            lists.push(rows.map(unifiedFromEpmRow))
            per.push({ source: 'europepmc', count: rows.length, total: data.hitCount || undefined })
          } catch (e) {
    ...
    (args, value) => {
      ...
      lines.push('Unified search "' + value.query + '" across ' + value.sources.join(' + ') + ' — ' + value.total + ' unique after de-duplication (' + per + ')')
