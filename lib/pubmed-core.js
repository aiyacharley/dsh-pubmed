...
function registerPubmedTools(ctx, deps) {
  ...
  register('pubmed_fetch_pdf_oa', 'Find open-access (OA) PDF links for a DOI or PMID — aggregates Unpaywall (authoritative OA status + every OA location), Europe PMC fullTextUrlList, and OpenAlex open_access. Returns a de-duplicated link list (direct PDF first, then landing pages) with host type, version (submitted/accepted/published), license, and OA status. Optionally downloads the best PDF to a local file with download:true (the PDF is NOT parsed — extraction is left to the caller). Use pubmed_fetch_fulltext for structured JATS text of PMC articles.',
    {
      doi: { type: 'string', description: 'DOI (e.g. "10.1038/nature12373"); provide exactly one of doi / pmid / pmcid' },
      pmid: { type: 'string', description: 'PubMed ID (resolved to a DOI/PMCID first)' },
      pmcid: { type: 'string', description: 'PMC ID (e.g. "PMC3531190")' },
      download: { type: 'boolean', default: false, description: 'Download the best PDF to a local file (requires the bundle plugin; returns savedPath + byte count). Off by default — review the link list first' },
      outDir: { type: 'string', description: 'Directory for the downloaded PDF (default: a dsh-pubmed-pdfs/ folder in the session workspace, i.e. <workspace>/dsh-pubmed-pdfs/)' },
    },
    async (args, exec) => {
      const wantDoi = normalizeDoi(args.doi)
      let pmid = args.pmid != null && String(args.pmid).trim() !== '' ? String(args.pmid).trim() : ''
      const rawPmcid = args.pmcid != null && String(args.pmcid).trim() !== '' ? String(args.pmcid).trim() : ''
      let pmcid = normalizePmcid(rawPmcid)
      // Count the RAW inputs, so an invalid-but-supplied id reports its own
      // validation error rather than "nothing provided".
      const provided = [args.doi, pmid, rawPmcid].filter((v) => v != null && String(v).trim() !== '').length
      if (provided === 0) throw new Error('Provide one of doi / pmid / pmcid')
      if (provided > 1) throw new Error('doi / pmid / pmcid are mutually exclusive — provide exactly one')
      if (args.doi && !wantDoi) throw new Error('Not a valid DOI: ' + args.doi)
      if (rawPmcid && !pmcid) throw new Error('Not a valid PMC ID: ' + rawPmcid)

      let doi = wantDoi
      // Resolve pmid/pmcid → doi (and fill the other id) so all three sources can run.
      if (!doi && (pmid || pmcid)) {
        try {
          if (!pmcid && pmid) pmcid = normalizePmcid(await pmidToPmcid(pmid, exec.signal))
          if (pmcid && !pmid) { try { pmid = String(await pmcidToPmid(pmcid, exec.signal) || '') } catch (e) { /* best-effort */ } }
          const idForDoi = pmid || pmid
          if (idForDoi) { try { doi = normalizeDoi(await pmidToDoi(idForDoi, exec.signal)) } catch (e) { /* best-effort */ } }
        } catch (e) { /* resolution is best-effort; PMC/EPMC sources still work */ }
      }

      const results = await Promise.allSettled([
        oaFromUnpaywall(doi, exec.signal),
        oaFromEpmc(doi, pmcid, exec.signal),
        oaFromOpenAlex(doi, exec.signal),
      ])
      const sourceErrors = []
      const groups = []
      const statuses = []
      ...
      const best = locations.find((l) => l.pdfUrl) || null
      ...
      // Optional download step — bytes to disk only, no parsing. Tries the
      // ranked PDF locations in order: publishers often answer a PDF URL with an
      // HTML consent/paywall page, so a failure here moves on to the next
      ...
      if (args.download) {
        ...
        if (!candidates.length) {
          out.download = { ok: false, error: 'no direct PDF URL found (see locations for landing pages)' }
        } else if (typeof deps.downloadToFile !== 'function') {
          out.download = { ok: false, error: 'download unavailable in this mode (requires the npm bundle plugin)' }
        } else {
          const attempts = []
          let done = null
          for (const url of candidates) {
            try {
              const dl = await deps.downloadToFile(url, { dir: args.outDir, signal: exec.signal })
              done = Object.assign({ ok: true, url }, dl)
              break
            } catch (e) {
              attempts.push({ url, error: String((e && e.message) || e) })
            }
          }
          if (done && attempts.length) done.attempts = attempts
          out.download = done || { ok: false, error: 'all ' + candidates.length + ' PDF link(s) failed', attempts }
        }
      }
      return out
    },
