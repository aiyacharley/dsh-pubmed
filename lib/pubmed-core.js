...
function registerPubmedTools(ctx, deps) {
  ...
  register('pubmed_search_papers', 'ONE search across PubMed + Europe PMC (both free), de-duplicated into a single ranked list — papers found on both platforms rank first, then per-platform relevance, then citations, then year. Per-source status is reported (one platform failing never kills the call). Use this for broad topic sweeps; use the platform tools (search_articles / europepmc_search) when you need their specific filters, sorting, or paging.',
    {
      ...
      sources: { type: 'array', items: { type: 'string', enum: ['pubmed', 'europepmc'] }, description: 'Which platforms to query; default = both' },
    },
    async (args, exec) => {
      const query = String(args.query || '').trim()
      if (!query) throw new Error('No query provided')
      const perSource = Math.min(50, Math.max(1, parseInt(args.maxResultsPerSource, 10) || 10))
      const wanted = (Array.isArray(args.sources) && args.sources.length) ? args.sources.filter((s) => ['pubmed', 'europepmc'].includes(s)) : ['pubmed', 'europepmc']
      if (!wanted.length) throw new Error('sources has no valid values — allowed: pubmed/europepmc')
      const lists = []
      const per = []
      ...
      return {
        ...
        sources: wanted,
        total: papers.length,
        papers: papers.slice(0, perSource * 2),
        perSource: per,
