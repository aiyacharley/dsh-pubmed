...
function registerPubmedTools(ctx, deps) {
  ...
  // Shared NCBI rate limiter: serializes every E-utilities request with a
  // ~350 ms gap so parallel tool calls cannot trip NCBI's 3 req/s limit
  // (which without an API key returns HTTP 429 and silently empties eSummary
  // steps). Mirrors the upstream server's global request queue.
  let ncbiChain = Promise.resolve()
  let ncbiLast = 0
  function ncbiScheduled(fn) {
    const run = ncbiChain.then(async () => {
      const now = Date.now()
      const wait = Math.max(0, ncbiLast + 350 - now)
      ncbiLast = Math.max(now, ncbiLast + 350)
