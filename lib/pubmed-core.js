...
function registerPubmedTools(ctx, deps) {
  ...
  function buildMermaid(gj, maxKeywords) {
    ...
    // also bring in the partners of directed relation edges so red arrows survive
    ...
    for (const e of gj.edges) {
      ...
      const sIn = chosen.has(e.source)
      const tIn = chosen.has(e.target)
      if (sIn && !tIn) { const n = nodeById.get(e.target); if (n) addNode(n, n.label) }
      else if (tIn && !sIn) { const n = nodeById.get(e.source); if (n) addNode(n, n.label) }
    }
