...
function registerPubmedTools(ctx, deps) {
  ...
  register('pubmed_pubtator_annotate', 'Fetch PubTator3 entity annotations (BioC JSON) for given PMIDs — typed entities (Gene / Chemical / Disease / Mutation / CellLine / Species) with concept IDs. Primary strategy for the knowledge graph; falls back to heuristic NLP when unavailable.',
    {
      pmids: { type: 'array', items: { type: 'string' }, required: true, description: 'PubMed IDs to annotate (1-100)' },
      full: { type: 'boolean', default: false, description: 'Also annotate the full text when available (default: title + abstract only)' },
    },
    async (args, exec) => {
      const pmids = asArray(args.pmids).map(String).filter(Boolean).slice(0, 100)
      if (!pmids.length) throw new Error('No PMIDs provided')
      let url = 'https://www.ncbi.nlm.nih.gov/research/pubtator3-api/publications/export/biocjson?pmids=' + encodeURIComponent(pmids.join(','))
      if (args.full) url += '&full=true'
      const r = await ncbiScheduled(() => httpGet(url, exec.signal, 60000))
  ...
  register('pubmed_pubtator_entity_id', 'Resolve a free-text bioconcept to candidate concept IDs via PubTator3 autocomplete (e.g. query=IgA → ncbi_gene:973 CD79A). Bridges heuristic keyword nodes to authoritative concept IDs.',
    {
    ...
    async (args, exec) => {
      ...
      const r = await ncbiScheduled(() => httpGet(url, exec.signal, 30000))
      let list = []
      try {
  ...
  // PubTator concept extraction is injectable too: the npm bundle passes a
  // function that calls PubTator3 (biocjson) and returns per-article entities
  // `{type, id, database, surface}`. Absent deps (dynamic mode / no network),
  // the graph engine simply has no concept layer — the heuristic layer still runs.
  const extractPubtator = deps.extractPubtator || (() => [])
  // Curated concept→concept relations (PubTator3 relations API). Injectable:
  // receives the article's concept list `[{nodeId, type, conceptId}]` and
  // returns `[{source, target, type, publications}]` with source/target as
  // concept node ids. The npm bundle wires this to pubmed_pubtator_relations;
  // absent deps, no curated relation layer is added.
  const extractPubtatorRelations = deps.extractPubtatorRelations || (() => [])
  ...
  function mergeGraph(graph, articles) {
    ...
    for (const a of asArray(articles)) {
      ...
      for (const ent of extractPubtator(a)) {
        ...
        cn.lastSeen = now
        articleConcepts.push({ nodeId, type, conceptId: cid })
      ...
      for (const rel of extractPubtatorRelations(articleConcepts)) {
