...
function registerPubmedTools(ctx, deps) {
  ...
  // Best-effort NCBI rate-limit spacer (~3 req/s without an API key).
  ...
  register('pubmed_lookup_citation', 'Resolve partial bibliographic references to PubMed IDs via NCBI ECitMatch. Deterministic matching for known references.',
    ...
    async (args, exec) => {
      ...
      const bdata = citations
        ...
        .join('\r')
      const url = EUTILS + '/ecitmatch.cgi?' + qs({ db: 'pubmed', retmode: 'xml', bdata })
      const text = (await httpGet(url, exec.signal)).body
      const parsed = []
  ...
  register('pubmed_lookup_mesh', 'Search and explore the MeSH (Medical Subject Headings) vocabulary — descriptor records with tree numbers, scope notes, and entry terms.',
    ...
    async (args, exec) => {
      const body = await eutilsGet('esearch.fcgi', { db: 'mesh', term: args.query, retmode: 'json', retmax: Math.min(25, Math.max(1, args.maxResults || 10)), tool: TOOL }, exec.signal)
      let es
      try { es = JSON.parse(body) } catch (e) { throw new Error('MeSH ESearch returned invalid JSON') }
      const er = es.esearchresult || {}
      const uis = asArray(er.idlist).map(String)
      const results = []
      if (uis.length) {
        const xml = await eutilsGet('efetch.fcgi', { db: 'mesh', id: uis.join(','), rettype: 'xml', retmode: 'xml' }, exec.signal)
        for (const rec of xmlTag(xml, 'DescriptorRecord')) {
          const ui = cleanXml(xmlTag(rec, 'DescriptorUI').join(' ')) || ''
          const name = cleanXml(xmlTag(rec, 'DescriptorName').join(' ')) || ''
          const treeNumbers = xmlTag(rec, 'TreeNumber').map(cleanXml)
          const scopeNote = cleanXml(xmlTag(rec, 'ScopeNote').join(' ')) || undefined
          const entryTerms = xmlTag(rec, 'EntryTerm').map((e) => cleanXml(e))
          results.push({ ui, name, treeNumbers, scopeNote, entryTerms })
        }
      }
    ...
    async (args, exec) => {
      ...
      return {
        ...
        results: results.map((r) => ({
          ...
          title: r.title, authorString: r.authorString, journalTitle: r.journalTitle,
      ...
      for (const r of records) {
        try {
          ...
          out.push({
            ...
            title: hit.title, authorString: hit.authorString, journalTitle: hit.journalTitle,
