...
function registerPubmedTools(ctx, deps) {
  ...
  async function defaultExtractPubtator(a) {
    ...
    try {
      ...
      const r = await ncbiScheduled(() => httpGet(url, undefined, 30000))
  ...
  async function defaultExtractPubtatorRelations(concepts) {
    ...
    try {
      ...
      for (const c of concepts.slice(0, 3)) {
        ...
        for (const rel of (Array.isArray(list) ? list : []).slice(0, 20)) {
          const tNode = rel && byAccession.get(String(rel.target || ''))
          if (!tNode || String(rel.source || '') !== c.accession) continue
