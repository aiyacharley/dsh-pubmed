...
function registerPubmedTools(ctx, deps) {
  ...
  function buildMermaid(gj, maxKeywords) {
    ...
    for (const n of articles) {
      ...
      addNode(n, 'article', (pm ? 'PMID ' + pm + ' · ' : '') + title)
    }
    for (const n of kws) addNode(n, 'kw', n.label)
    if (articles.length) lines.push('  classDef article fill:#E64B35FF,color:#fff,stroke:#C0392B')
    if (kws.length) lines.push('  classDef kw fill:#00A087FF,color:#fff,stroke:#00897B')
    if (articles.length) lines.push('  class ' + articles.map((n) => chosen.get(n.id)).join(',') + ' article')
    if (kws.length) lines.push('  class ' + kws.map((n) => chosen.get(n.id)).join(',') + ' kw')
    let edgeIndex = 0
    const undIds = []
    const relIds = []
