  ...
  register('pubmed_search_papers', 'ONE search across PubMed + Europe PMC (+ optional Semantic Scholar), de-duplicated into a single ranked list — papers found on multiple platforms rank first, then per-platform relevance, then citations, then year. Per-source status is reported (one platform failing never kills the call). Add "s2" to sources to include Semantic Scholar, or pass sources:["all"] for everything (opt-in s2: slower — ~3s/call without an S2 key — and uses the shared S2 budget). Optionally filter by year and re-sort by citations/year. Use this for broad topic sweeps; use the platform tools (search_articles / europepmc_search / search_s2) when you need their specific filters, sorting, or paging.',
    {
      ...
      sources: { type: 'array', items: { type: 'string', enum: ['pubmed', 'europepmc', 's2', 'all'] }, description: 'Which platforms to query; "all" = pubmed+europepmc+s2; default = pubmed+europepmc' },
    ...
    async (args, exec) => {
      ...
      let wanted = (Array.isArray(args.sources) && args.sources.length) ? args.sources.filter((s) => ['pubmed', 'europepmc', 's2', 'all'].includes(s)) : ['pubmed', 'europepmc']
      if (wanted.includes('all')) wanted = ['pubmed', 'europepmc', 's2']
      if (!wanted.length) throw new Error('sources has no valid values — allowed: pubmed/europepmc/s2/all')
      const lists = []
      const per = []
