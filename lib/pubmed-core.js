...
function registerPubmedTools(ctx, deps) {
  ...
  // F2 (round 2): relation endpoints must be stopword-free — mid-span stopwords
  // ("microbiome by microbial") still produced junk keyword nodes that survived
  // minCount via the force-keep. Legit endpoints ("bile acid metabolism") have
  // none, so this is a safe quality gate for the relation path.
  ...
  // F2: relation spans are regex-captured and regularly carry stopword debris
  // ("they share similar", "are", "and") — these become keyword nodes through the
  // relation-edge path and survive minCount filtering (relation endpoints are
  // force-kept). Trim leading/trailing stopwords and drop content-free spans.
  function cleanRelationSpan(span, stopwords) {
    let words = String(span || '').toLowerCase().split(/\s+/).filter(Boolean)
